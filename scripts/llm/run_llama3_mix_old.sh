...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    if [ "$FEW_SHOT" == "None" ]; then
        SAVE_FILENAME="answers/${DATASET_NAME}/food2k_3_rank_old_${checkpoint_iter}.jsonl"
        ...
        SAVE_FILENAME="answers/${DATASET_NAME}/food2k_3_rank_old_${checkpoint_iter}_fewshot${FEW_SHOT}.jsonl"
...
fewshot_number=None
gpu=0
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_old.sh $(( (gpu + 1) * 1000 )) "$fewshot_number" > log/llama3_food2k_3_rank_old_$gpu.log &
gpu=1
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_old.sh $(( (gpu + 1) * 1000 )) "$fewshot_number" > log/llama3_food2k_3_rank_old_$gpu.log &
gpu=2
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_old.sh $(( (gpu + 1) * 1000 )) "$fewshot_number" > log/llama3_food2k_3_rank_old_$gpu.log &
...
fewshot_number=4
gpu=4
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_old.sh $(( (gpu - 3) * 1000 )) "$fewshot_number" > log/llama3_food2k_3_rank_old_$gpu.log &
gpu=5
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_old.sh $(( (gpu - 3) * 1000 )) "$fewshot_number" > log/llama3_food2k_3_rank_old_$gpu.log &
gpu=6
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_old.sh $(( (gpu - 3) * 1000 )) "$fewshot_number" > log/llama3_food2k_3_rank_old_$gpu.log &
