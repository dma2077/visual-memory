...
if [ "$checkpoint_iter" == "None" ]; then
    ...
    MODEL_PATH="/map-vepfs/dehua/model/checkpoints/llama3/food2k_3_rank_old/checkpoint-${checkpoint_iter}"
    echo $MODEL_PATH
fi
...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    if [ "$FEW_SHOT" == "None" ]; then
        SAVE_FILENAME="answers/${DATASET_NAME}/food2k_3_rank_old_${checkpoint_iter}_mixeddataset.jsonl"
        ...
        SAVE_FILENAME="answers/${DATASET_NAME}/food2k_3_rank_old_${checkpoint_iter}_fewshot${FEW_SHOT}_mixeddataset.jsonl"
    ...
    # 运行 Python 脚本
    python code/llm/llama3_vllm_text_mix.py \
...
fewshot_number=None
gpu=0
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_mixdataset.sh 7000 "$fewshot_number" > gpu_$gpu.log &
...
# gpu=1
# CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/llm/run_llama3_mix_mixdataset.sh $(( (gpu + 1) * 1000 )) "$fewshot_number" > gpu_$gpu.log &
# gpu=2
