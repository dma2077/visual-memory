...
MODEL_PATH='/map-vepfs/models/llava-hf/llava-1.5-7b-hf'
FEW_SHOT="$1"         # 第二组参数
...
if [ "$FEW_SHOT" == "None" ]; then
    ...
    SAVE_FILENAME="answers/${DATASET_NAME}/llava1.5-${k1}.jsonl"
...
# 运行 Python 脚本
python code/vlm/llava1.5_text_rank.py \
...
gpu=0
k1=5
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/vlm/llava1.5/run_llava1.5_raw.sh "$fewshot_number" "$k1" "$DATASET_NAME" > log/llava1.5_${DATASET_NAME}_${k1}.log 2>&1
