...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    for checkpoint_iter in "${checkpoints[@]}"; do
        if [ "$FEW_SHOT" == "None" ]; then
            MODEL_PATH="/map-vepfs/dehua/model/checkpoints/qwen2vl/food101_raw/checkpoint-${checkpoint_iter}"
...
fewshot_number=None
gpu=0
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/vlm/run_qwen2vl_raw.sh "$fewshot_number" > log/qwen2vl_raw_food101_None.log &
