...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    if [ "$FEW_SHOT" == "None" ]; then          
        SAVE_FILENAME="answers/${DATASET_NAME}/rar_mixedtrain.jsonl"
        DINO_FILE="questions/${DATASET_NAME}/clip_test_${K}_mixedtrain.jsonl"
        SIGLIP_FILE="questions/${DATASET_NAME}/siglip_test_${K}_mixedtrain.jsonl"
    else
        SAVE_FILENAME="answers/${DATASET_NAME}/rar_mixedtrain_fewshot${FEW_SHOT}.jsonl"
        DINO_FILE="questions/${DATASET_NAME}/clip_test_${K}_mixedtrain_fewshot${FEW_SHOT}.jsonl"
...
fewshot_number=8
gpu=1
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/vlm/run_rar_mix_dataset.sh "$fewshot_number" > rar_mix_dataset_$fewshot_number.log 2>&1 &
