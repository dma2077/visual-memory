...
MODEL_PATH='/map-vepfs/models/llava-hf/llava-v1.6-mistral-7b-hf'
FEW_SHOT="$1"         # 第二组参数
...
if [ "$FEW_SHOT" == "None" ]; then
    ...
    SAVE_FILENAME="answers/${DATASET_NAME}/llava-next-mixtral-${k1}.jsonl"
...
gpu=0
k1=5
CUDA_VISIBLE_DEVICES=$gpu nohup bash scripts/vlm/llava-next/run_llava_next_raw1.sh "$fewshot_number" "$k1" "$DATASET_NAME" > log/llava_next1_101_${k1}.log 2>&1 &
