...
prompt_template1 = """
We use one retrievers to identify the category of the current image. The food categories returned by the first retriever, listed in descending order of similarity, are {content1}. Please make a comprehensive judgment on the final category of the image based on the current image and the results from the retriever. Only provide the category name; no need to include other details.
"""

def get_food_prompts_single_model(filename, data_proportion, k1):
    test_data1 = load_jsonl(filename)
        ...
        conversation = [
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": prompt_template1.format(content1=content1)},
                ],
            }
        ]
...
def get_siglip_food_prompts(filename, data_proportion):
    ...
    for idx, data in tqdm(
        enumerate(test_data), total=len(test_data), desc="Loading test data"
    ):
        # Add final user question
...
def get_food_prompts_multi_model(filename1, filename2, data_proportion, k1):
    ...
    for idx, data in tqdm(
        enumerate(test_data1), total=len(test_data1), desc="Loading test data"
    ):
        # Add final user question
        categories1 = data["categories"][:k1]
...
def main(args):
    ...
    if args.mode == "mix":
        messages, image_path_list = get_food_prompts_multi_model(
            question_file, siglip_file, args.data_proportion, args.k1
        )
    elif args.mode == "dinov2":
        messages, image_path_list = get_food_prompts(
            question_file, args.data_proportion
        )
    elif args.mode == "siglip":
        messages, image_path_list = get_food_prompts_single_model(
            question_file, args.data_proportion, k1=args.k1
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run VLM inference with specified parameters."
    )
    parser.add_argument(
        "--model_path", type=str, required=True, help="Path to the model checkpoint."
    )
    parser.add_argument(
        "--save_filename", type=str, required=True, help="Filename to save the results."
    )
    parser.add_argument(
        "--gold_answer_file_path",
        type=str,
        required=True,
        help="Path to the gold answer file.",
    )
    parser.add_argument(
        "--label_file", type=str, required=True, help="Path to the label file."
    )
    parser.add_argument(
        "--question_file", type=str, required=True, help="Path to the question file."
    )
    parser.add_argument(
        "--siglip_file", type=str, required=True, help="Path to the siglip file."
    )
    parser.add_argument(
        "--data_proportion",
        type=float,
        default=1.0,
        help="Proportion of data to use (0.0 to 1.0).",
    )
    parser.add_argument(
        "--mode",
        type=str,
