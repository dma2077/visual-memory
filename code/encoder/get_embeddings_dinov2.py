...
def get_dataset_paths(dataset_name):
    ...
    elif dataset_name == 'vft-lt':
        root_dir = '/map-vepfs/dehua/data/data/vft-lt/'
        train_file = '/map-vepfs/dehua/data/data/vft-lt/train.txt'
        test_file = '/map-vepfs/dehua/data/data/vft-lt/test.txt'
    else:
...
def main():
    ...
    # 获取模型最后一层的特征维度
    with torch.no_grad():
        dummy_input = torch.randn(1, 3, 224, 224).to(device)  # 假设输入图像大小为 224x224
...
def parse_args():
    ...
    parser.add_argument('--checkpoint_path', type=str, help="Path to the model checkpoint (.pth file)", default=None)
