...
def main(args):
    ...
    datasets = ["food101", "food2k", "food172", "fru92", "veg200", 'foodx251']
    merged_dict = {}
    current_key = 0
    for dataset_name in datasets:
        label_file = get_label_file(dataset_name)
        id2category_dict_single = build_id2category_dict(label_file, dataset_name)
        for key in sorted(id2category_dict_single.keys()):
            merged_dict[current_key] = id2category_dict_single[key]
            current_key += 1
    id2category_dict = merged_dict
    category2id_dict = {v: f"category{k}" for k, v in id2category_dict.items()}
    print(category2id_dict)
