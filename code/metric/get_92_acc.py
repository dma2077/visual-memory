...
def build_fru92_id2category(label_file):
    """
    构建 fru92 的 ID→类别 映射。
...
def evaluate_fru92(pred_file, label_file, save_mismatch_json=None, extract_answer=False, supclass=False, attribute=False):
    """
    评估 fru92 数据集预测准确率。
    - pred_file: JSONL 文件，每行包含 "text" 和 "image"。
    - label_file: fru92 的类别列表文件。
    - save_mismatch_json: 若指定，则把所有不匹配项写入该文件。
    - extract_answer: 是否抽取 <answer> 标签内的内容作为预测。
    返回 (accuracy, total, correct)。
    """
    # 构建类别映射
    id2cat = build_fru92_id2category(label_file)
    ...
    with open(pred_file, 'r', encoding='utf-8') as f:
        for line in tqdm(f, desc="Evaluating fru92"):
    ...
    print(f"fru92 accuracy: {accuracy:.4f} ({correct}/{total})")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python evaluate_fru92.py <prediction_file>")
        sys.exit(1)
    
    prediction_file = sys.argv[1]
    label_file = "/llm_reco/dehua/data/food_data/fru92_lists/fru_subclasses.txt"
    mismatch_output = "/llm_reco/dehua/code/visual-memory/answers/fru92/mismatches_fru92.json"
    extract_ans = False
    supclass = False
    attribute = True
