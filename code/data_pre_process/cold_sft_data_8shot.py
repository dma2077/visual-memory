...
result_paths = {
    "food101": "/llm_reco/dehua/data/food_finetune_data/food101_cold_sft_8shot.json",
    "food172": "/llm_reco/dehua/data/food_finetune_data/food172_cold_sft_8shot.json",
    "foodx251": "/llm_reco/dehua/data/food_finetune_data/foodx251_cold_sft_8shot.json",
    "food2k": "/llm_reco/dehua/data/food_finetune_data/food2k_cold_sft_8shot.json",
    "veg200": "/llm_reco/dehua/data/food_finetune_data/veg200_cold_sft_8shot.json",
    "fru92": "/llm_reco/dehua/data/food_finetune_data/fru92_cold_sft_8shot.json"
}
file_paths1 = {
...
def sample_few_shot_data(data_list, few_shot=8, seed=42):
    """
    对数据进行8-shot采样，确保每个类别最多包含8个样本
    
    Args:
        data_list: 原始数据列表
        few_shot: 每个类别的最大样本数量，固定为8
        seed: 随机种子
...
if __name__ == "__main__":
    print("=== Food Dataset 8-Shot Processing ===")
    ...
    for path in tqdm(file_paths, desc="处理 JSON 文件", unit="file"):
        ...
        # 进行8-shot采样
        print(f"[INFO] Applying 8-shot sampling...")
        sampled_data = sample_few_shot_data(data_list, few_shot=8)
    ...
    print(f"\n✅ 8-Shot processing completed. Missing image count: {len(missing_paths)}") 