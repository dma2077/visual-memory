...
def get_similarity_categories(retrieval_matrix, query_category, value_category, k_max):
    ...
    for i in range(len(query_category)):
        ...
        assert len(retrieved_indices) == 5
...
category_matrix_train = get_similarity_categories(I_train, category_train, category_train, k)
category_matrix_test = get_similarity_categories(I_test, category_test, category_train, k)
similarity_matrix_train = get_similarities(D_train, category_train, category_train, k)
similarity_matrix_test = get_similarities(D_test, category_test, category_train, k)
...
# def save_train_retrieval_prompt(category_matrix, similarity_matrix, retrieval_save_path):
#     conversation_list = []
...
#             {
#                 "from": "human",
#                 "value": f"<image>\n{prompt_template.format(category1=category_template[0], similarity1=similarity_template[0], category2=category_template[1], similarity2=similarity_template[1], category3=category_template[2], similarity3=similarity_template[2], category4=category_template[3], similarity4=similarity_template[3], category5=category_template[4], similarity5=similarity_template[4])}"
#             },
#             {
#                 "from": "gpt",
#                 "value": k.split('/')[-2].replace('_', ' ')
#             }
#             ]
#         }
...
def save_train_retrieval_prompt(category_matrix, similarity_matrix, retrieval_save_path):
    ...
    for idx, (k, v) in enumerate(category_matrix.items()):
        ...
        conversation = {
            "messages": [
                {
                    "content": f"<image>{prompt_template.format(max_k=k, category1=category_template[0], category2=category_template[1], category3=category_template[2], category4=category_template[3], category5=category_template[4])}",
                    # similarity1=similarity_template[0], similarity2=similarity_template[1], similarity3=similarity_template[2], similarity4=similarity_template[3], similarity5=similarity_template[4]
                    "role": "user"
                },
                {
                    "content": k.split('/')[-2].replace('_', ' '),
                    "role": "assistant"
                },
            ],
            "images": [
...
retrieval_trian_path = f'/mnt/madehua/fooddata/json_file/sharegpt/101_train_{max_k}.json'
# retrieval_test_path = '/mnt/madehua/fooddata/json_file/172_questions_retrieval_5_v1.jsonl'
...
#save_train_retrieval_prompt(category_matrix_train, similarity_matrix_train, retrieval_trian_path)
#save_test_retrieval_prompt(category_matrix_test, similarity_matrix_train, retrieval_test_path)